...
from trimf.entities import Document, Dataset, EntityType
from trimf.input_reader import JsonInputReader
import jinja2
...
class Evaluator:
    def __init__(self, dataset: Dataset, input_reader: JsonInputReader, text_encoder: BertTokenizer,
        ...
        self._ent_attentions=[]
        self._rel_attentions=[]
        self._trigger_attn=[]
    ...
    def eval_batch(self, batch_entity_clf: torch.tensor, batch_rel_clf: torch.tensor,
                   batch_rels: torch.tensor, batch: dict, ent_attention,rel_attention, trigger_attn):
        ...
        for i in range(batch_size):
            ...
            if ent_attention:
                self._ent_attentions.append(ent_attention[i])
            if rel_attention:
                self._rel_attentions.append(rel_attention[i])
            if trigger_attn:
                self._trigger_attn.append(trigger_attn[i])
    ...
    def store_examples(self):
        ...
        for i, doc in enumerate(self._dataset.documents):
            ...
            if len(self._ent_attentions)==0:
                ent_attention=[0]
            else:
                ent_attention=self._ent_attentions[i]
            if len(self._rel_attentions)==0:
                rel_attention=[0]
            else:
                rel_attention=self._rel_attentions[i]
            
            if len(self._trigger_attn)==0:
                trigger_attn=[0]
            else:
                trigger_attn=self._trigger_attn[i]
            # entities
            entity_example = self._convert_example(doc, self._gt_entities[i], self._pred_entities[i], ent_attention,rel_attention, trigger_attn,
                                                   include_entity_types=True, to_html=self._entity_to_html)
            entity_examples.append(entity_example)

            # relations
            # without entity types
            rel_example = self._convert_example(doc, self._gt_relations[i], self._pred_relations[i],ent_attention,rel_attention, trigger_attn,
                                                include_entity_types=False, to_html=self._rel_to_html)
            rel_examples.append(rel_example)

            # with entity types
            rel_example_nec = self._convert_example(doc, self._gt_relations[i], self._pred_relations[i],ent_attention,rel_attention, trigger_attn,
    ...
    def _convert_example(self, doc: Document, gt: List[Tuple], pred: List[Tuple],ent_attention,rel_attention, trigger_attn,
        ...
        ent_attn=list(zip(doc.tokens,ent_attention))
        rel_attn=list(zip(doc.tokens,rel_attention))
        trigger_attn=list(zip(doc.tokens,trigger_attn))
        e_tokens=[t._phrase for e in doc.entities for t in e._tokens]
        e_tokens.extend([",",".","`",";","an",":","a","the","'"])
        trigger_attn=list(filter(lambda x:x[0]._phrase not in e_tokens, trigger_attn))
        rel_triggers=sorted(trigger_attn,key=lambda x:x[1],reverse = True )

        return dict(text=text, tp=tp, fn=fn, fp=fp, precision=precision, recall=recall, f1=f1, length=len(doc.tokens),ent_attn=ent_attn,rel_attn=rel_attn,rel_triggers=rel_triggers)
