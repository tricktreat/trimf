...
def create_train_sample(doc, neg_entity_count: int, neg_rel_count: int, max_span_size: int, rel_type_count: int):
    ...
    dependency_tree = doc.dependency_tree
    edge_index = dependency_tree.edge_index
    edge_type = dependency_tree.edge_type
    num_nodes = len(doc.tokens)

    dt_data = Data(x=None,edge_index=torch.tensor(edge_index))
    dt_data.edge_type = torch.tensor(edge_type)
    dt_data.num_nodes = num_nodes
    ...
    for e in doc.entities:
        ...
        pos_entity_masks_token.append(create_entity_mask(*e.span_token, num_nodes))
    ...
    neg_entity_samples = random.sample(list(zip(neg_entity_spans, neg_entity_spans_token, neg_entity_sizes)),
                                       min(len(neg_entity_spans), neg_entity_count))
    neg_entity_spans, neg_entity_spans_token, neg_entity_sizes = zip(*neg_entity_samples) if neg_entity_samples else ([], [], [])
    ...
    neg_entity_masks_token = [create_entity_mask(*span, num_nodes) for span in neg_entity_spans_token]
    ...
    token_m = torch.ones(token_count, dtype=torch.bool)
    ...
    if entity_masks:
        ...
        # entity_sample_masks_token = torch.ones([entity_masks_token.shape[0]], dtype=torch.bool)
    else:
        ...
        entity_masks_token = torch.zeros([1, num_nodes], dtype=torch.bool)
        # entity_sample_masks_token = torch.zeros([entity_masks_token.shape[0]], dtype=torch.bool)
    ...
    # print(entity_masks_token,rels)
    return dict(encodings=encodings, context_masks=context_masks,token_m=token_m, token_masks=token_masks, 
                entity_masks=entity_masks, entity_masks_token = entity_masks_token, entity_sizes=entity_sizes, entity_types=entity_types,
                rels=rels, rel_masks=rel_masks, rel_types=rel_types_onehot,
                token_sample_masks=token_sample_mask, entity_sample_masks=entity_sample_masks, rel_sample_masks=rel_sample_masks, dt=dt_data)
...
def create_eval_sample(doc, max_span_size: int):
    ...
    for size in range(1, max_span_size + 1):
        for i in range(0, (token_count - size) + 1):
            ...
            entity_masks_token.append(create_entity_mask(*span_token, num_nodes))
    ...
    return dict(encodings=encodings, context_masks=context_masks,token_m=token_m,token_masks=token_masks, entity_masks=entity_masks, entity_masks_token= entity_masks_token,
                entity_sizes=entity_sizes, entity_spans=entity_spans, entity_sample_masks=entity_sample_masks,dt=dt_data)
...
def collate_fn_padding(batch):
    ...
    for key in keys:
        if key=="dt":
            padded_batch[key+"_data_list"] = [s[key] for s in batch]
            continue
