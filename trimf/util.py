...
from trimf.entities import TokenSpan

CSV_DELIMETER = ';'


def detect_nan(your_tensor):
    return torch.isnan(your_tensor).any()
...
def masked_softmax(vec, mask, dim=1, epsilon=1e-25):
    # vec[~mask]=float("-inf")
    vec[~mask]=-1e25
    return torch.nn.functional.softmax(vec,dim=dim)
...
def check_version(config, model_class, model_path):
    if os.path.exists(model_path):
        ...
        # version check
        loaded_version = config_dict.get('trimf_version', '1.0')
        if 'rel_classifier.weight' in state_dict and loaded_version != model_class.VERSION:
            msg = ("Current TriMF version (%s) does not match the version of the loaded model (%s). "
