...
from trimf.entities import Dataset, EntityType, RelationType, Entity, Relation, Document
...
class JsonInputReader(BaseInputReader):
    ...
    def _parse_document(self, doc, dataset) -> Document:
        ...
        doc_tokens, doc_encoding = self._parse_tokens(jtokens, ltokens, rtokens, dataset)

        # parse entity mentions
        entities = self._parse_entities(jentities, doc_tokens, dataset)

        dt = self._parse_dep_tree(jdenps, doc_tokens, dataset)

        # parse relations
        relations = self._parse_relations(jrelations, entities, dataset)

        # create document
        document = dataset.create_document(doc_tokens, dt, entities, relations, doc_encoding)
    ...
    def _parse_dep_tree(self, jdenp, doc_tokens,dataset):
        token_pairs=[(doc_tokens[jd["head"]],doc_tokens[jd["tail"]]) for jd in jdenp]
        types=[jd["type"] for jd in jdenp]
        return dataset.create_dep_tree(token_pairs, types)