...
from trimf import models
from trimf import sampling
from trimf import util
from trimf.entities import Dataset
from trimf.evaluator import Evaluator
from trimf.input_reader import JsonInputReader, BaseInputReader
from trimf.loss import TriMFLoss, Loss
from tqdm import tqdm
from trimf.trainer import BaseTrainer

SCRIPT_PATH = os.path.dirname(os.path.realpath(__file__))

# torch.multiprocessing.set_start_method("spawn",force=True)

class TriMFTrainer(BaseTrainer):
    ...
    def train(self, train_path: str, valid_path: str, types_path: str, input_reader_cls: BaseInputReader):
        ...
        model = model_class.from_pretrained(self.args.model_path,
                                            ...
                                            device = self._device,
                                            syn_graph= self.args.syn_graph, 
                                            sema_graph=self.args.sema_graph, 
                                            fusion_rgcn = self.args.fusion_rgcn, 
                                            tw_grad_flow_token = self.args.tw_grad_flow_token, 
                                            tw_grad_flow_subword = self.args.tw_grad_flow_subword, 
                                            tw_rel_atten_token=self.args.tw_rel_atten_token, 
                                            tw_ent_atten_token=self.args.tw_ent_atten_token, 
                                            tw_rel_atten_subword=self.args.tw_rel_atten_subword, 
                                            tw_ent_atten_subword=self.args.tw_ent_atten_subword, 
                                            trigger_attn = self.args.trigger_attn,
                                            trigger_grad_flow=self.args.trigger_grad_flow,
                                            full_graph_retain_rate=self.args.full_graph_retain_rate,
                                            dt_graph_retain_rate=self.args.dt_graph_retain_rate,
                                            split_epoch=self.args.split_epoch )
        ...
        scheduler = torch.optim.lr_scheduler.OneCycleLR(optimizer, max_lr=args.lr,total_steps=updates_total)                                                 
        # create loss function
        rel_criterion = torch.nn.BCEWithLogitsLoss(reduction='none')
        entity_criterion = torch.nn.CrossEntropyLoss(reduction='none')
        compute_loss = TriMFLoss(rel_criterion, entity_criterion, model, optimizer, scheduler, args.max_grad_norm)
        ...
        self._eval(model, validation_dataset, input_reader, args.epochs, updates_epoch, save=True)
        # save final model
        extra = dict(epoch=args.epochs, updates_epoch=updates_epoch, epoch_iteration=0)
    ...
    def eval(self, dataset_path: str, types_path: str, input_reader_cls: BaseInputReader):
        ...
        # evaluate
        self._eval(model, input_reader.get_dataset(dataset_label), input_reader, save=True)
    ...
    def _train_epoch(self, model: torch.nn.Module, compute_loss: Loss, optimizer: Optimizer, dataset: Dataset,
        ...
        for batch in tqdm(data_loader, total=total, desc='Train epoch %s' % epoch):
            ...
            # forward step
            entity_logits, rel_logits = model(encodings=batch['encodings'], context_masks=batch['context_masks'],token_m=batch['token_m'],token_masks=batch['token_masks'],
                                              dt_data_list=batch['dt_data_list'],entity_masks=batch['entity_masks'], entity_masks_token=batch['entity_masks_token'], entity_sizes=batch['entity_sizes'],
            ...
            # logging
            iteration += 1
            global_iteration = (epoch-1) * updates_epoch + iteration
    ...
    def _eval(self, model: torch.nn.Module, dataset: Dataset, input_reader: JsonInputReader,
              epoch: int = 0, updates_epoch: int = 0, iteration: int = 0, save: bool =False):
        ...
        with torch.no_grad():
            ...
            for batch in tqdm(data_loader, total=total, desc='Evaluate epoch %s' % epoch):
                ...
                result = model(encodings=batch['encodings'], context_masks=batch['context_masks'],token_m=batch['token_m'],
                               token_masks=batch['token_masks'],dt_data_list=batch['dt_data_list'],entity_masks=batch['entity_masks'], entity_masks_token=batch['entity_masks_token'],  entity_sizes=batch['entity_sizes'],
                               entity_spans=batch['entity_spans'], entity_sample_masks=batch['entity_sample_masks'],
                               evaluate=True)
                entity_clf, rel_clf, rels, ent_attention,rel_attention, trigger_attn= result

                # evaluate batch
                evaluator.eval_batch(entity_clf, rel_clf, rels, batch, ent_attention,rel_attention, trigger_attn)

        global_iteration = epoch * updates_epoch + iteration
        ner_eval, rel_eval, rel_nec_eval = evaluator.compute_scores()
        # only previous eval and test log tb
        if save!=True or  dataset.label=="test":
            self._log_eval(*ner_eval, *rel_eval, *rel_nec_eval,
                        epoch, iteration, global_iteration, dataset.label)

        if self.args.store_predictions and not self.args.no_overlapping and save:
            evaluator.store_predictions()

        if self.args.store_examples and save:
