...
class Deptree:
    types = ['oprd', 'acl', 'neg', 'compound', 'advcl', 'advmod', 'attr', 'ROOT', 'subtok', 'preconj', 'ccomp', 'nmod', 'xcomp', 'intj', 'npadvmod', 'aux', 'punct', 'det', 'predet', 'mark', 'amod', 'pobj', 'pcomp', 'conj', 'nsubjpass', 'auxpass', 'dep', 'acomp', 'appos', 'quantmod', 'parataxis', 'prt', 'prep', 'dative', 'dobj', 'nummod', 'csubj', 'nsubj', 'meta', 'case', 'relcl', 'expl', 'csubjpass', 'cc', 'agent', 'poss']
    type_idx = dict((it,idx) for idx,it in enumerate(types))

    def __init__(self, token_pairs: List[Tuple[Token,Token]], types: List[str]):
        self._token_pairs = token_pairs
        # print(Deptree.type_idx)
        self._types =[Deptree.type_idx[t] for t in types]
    
    @property
    def sentence(self):
        return [item[0] for item in self._token_pairs]
    ...
    def edge_index(self):
        # 2 * N
        return [[pair[0].index for pair in self._token_pairs], [pair[1].index for pair in self._token_pairs]]



class Document:
    def __init__(self, doc_id: int, tokens: List[Token], dt: Deptree,entities: List[Entity], relations: List[Relation],
                 encoding: List[int]):
...
class Dataset(TorchDataset):
    ...
    def create_token(self, idx, span_start, span_end, phrase) -> Token:
        ...
        self._tokens[self._tid] = token
        self._tid += 1
        return token

    def create_document(self, tokens, dt, entity_mentions, relations, doc_encoding) -> Document:
        document = Document(self._doc_id, tokens, dt, entity_mentions, relations, doc_encoding)
    ...
    def create_dep_tree(self, token_pairs, types):
        dt = Deptree(token_pairs, types)
        self._dts[self._did] = dt
        self._did += 1
        return dt
